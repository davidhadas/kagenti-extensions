...
| [`ctx-logger`](ctxlogger/README.md) | Diagnostic: logs the pipeline `Context` (esp. `Session`) per leg/phase at INFO. Read-only, never blocks. `authbridge-proxy` only — no envoy-sidecar shim |
