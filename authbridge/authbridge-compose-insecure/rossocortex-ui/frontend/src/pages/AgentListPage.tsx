      ...
      {agentQuery.data && agentQuery.data.length > 0 && (
        <Gallery hasGutter minWidths={{ default: '300px' }}>
          {agentQuery.data.map((agent: Agent) => (
            <GalleryItem key={`${agent.namespace}/${agent.name}`} style={{ height: '100%' }}>
              <Card
                isClickable
                isSelectable
                onClick={() => navigate(`/chat/${agent.namespace}/${agent.name}`, { state: { agent } })}
                    ...
                    <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      ...
                      {agent.security && (
                        <LockIcon
                          ...
                          style={{ color: '#6a6e73', fontSize: '12px' }}
                        />
                      )}
                    </span>
                ...
                <CardBody>
                  ...
                  {agent.tags && agent.tags.length > 0 && (
                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginTop: '10px' }}>
                      {agent.tags.map((tag) => (
                        <Label key={tag} color="blue" isCompact>{tag}</Label>
                      ))}
                    </div>
                  )}
                </CardBody>
