...
Each agent is isolated on its own `*-internal` network and reaches the outside
world (and is reached) only through its AuthBridge instance, which bridges that
internal network to a `shared` network carrying the common services (UI,
token-broker, bundle-server, GitHub MCP).
...
rossocortex-frontend :3000  ──►  rossocortex-backend (BFF)
  (React SPA)                      │  POST /api/v1/chat/...
                          ...
                          authbridge-<agent> :8080  (reverse proxy, shared net)
                                   │
                            agent-<agent> :8000     (git-issue-internal net, isolated)
...
cd kagenti-extensions/authbridge/authbridge-compose
...
├── agents/                 # one file per agent (its internal network + authbridge + agent + registrar)
...
| `authbridge-<agent>` | 9000→8080 | Reverse proxy (inbound to agent); 8081 forward proxy (internal) |
...
1. Copy an existing agent file:
   ...
   ```
   In `agents/<name>.yaml`:
   - Rename the internal network (`git-issue-internal` → `<name>-internal`) and
     the three services (`authbridge-<name>`, `agent-<name>`, `registrar-<name>`).
